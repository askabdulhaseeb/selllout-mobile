      ...
      body: Consumer<ProductProvider>(
        builder: (BuildContext context, ProductProvider prodPro, _) {
          final List<Product> prods = prodPro.productsByUsers(
              Provider.of<UserProvider>(context, listen: false).user(uid: AuthMethods.uid));
          prods.shuffle();
          return prodPro.isLoading
              ...
              : ListView.separated(
                  ...
                  separatorBuilder: (BuildContext context, int index) =>
                      Divider(thickness: 4, color: Theme.of(context).textTheme.bodyLarge!.color!.withOpacity(0.08)),
                  itemBuilder: (BuildContext context, int index) => prods.length == index ?
                  Padding(padding: const EdgeInsets.symmetric(vertical: 48),
                    child: Column(children: <Widget>[
                        const Icon(Icons.production_quantity_limits, size: 48,),
                      const SizedBox(height: 10),
                      const Text('No More Products available to watch', style: TextStyle(color: Colors.grey)),
                      Consumer<UserProvider>(builder:
                        ...
                        return TextButton(
                          ...
                          child: const Text('Explore more', style:
